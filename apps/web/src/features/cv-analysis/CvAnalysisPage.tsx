...
export function CvAnalysisPage() {
  ...
  if (status === 'processing') {
    return (
      <PageTransition><div className="mx-auto max-w-2xl space-y-6">
        <h1 className="text-2xl font-bold text-white">CV Analysis</h1>
        <Card variant="glass" className="flex flex-col items-center py-16 text-center">
          <Loader2 className="h-12 w-12 animate-spin text-brand-500" />
          <h3 className="mt-4 text-lg font-medium text-white">Analyzing your CV...</h3>
          <p className="mt-1 text-sm text-brand-200">This usually takes 10-30 seconds.</p>
          <div className="mt-8 flex w-full max-w-sm flex-col gap-2">
            {['Extracting text', 'Checking quality', 'AI analysis'].map((step, i) => (
              <div key={step} className="flex items-center gap-3 text-sm">
                <div className={`h-6 w-6 rounded-full ${i < 2 ? 'bg-success-500/20 text-success-300' : 'bg-brand-500/20 text-brand-300'} flex items-center justify-center text-xs font-medium`}>
                  {i < 2 ? <CheckCircle2 className="h-4 w-4" /> : <Loader2 className="h-4 w-4 animate-spin" />}
                </div>
                <span className={i < 2 ? 'text-brand-200' : 'font-medium text-white'}>{step}</span>
              </div>
            ))}
          </div>
        </Card>
  ...
  if (analysisResult) {
    ...
    const sevClass: Record<string, string> = {
      ...
      important: 'border-warning-500/30 bg-warning-500/10 text-warning-300',
      minor: 'border-brand-500/30 bg-brand-500/10 text-brand-300',
    };
    ...
    return (
      <PageTransition><div className="mx-auto max-w-2xl space-y-6">
        ...
        <Card variant="glass">
          <p className="text-sm text-brand-100">{analysisResult.overallSummary}</p>
          {analysisResult.strengths?.length > 0 && (
            <div className="mt-4 space-y-2">
              <h4 className="font-semibold text-white">Strengths</h4>
              {analysisResult.strengths.map((s: string, i: number) => (
                <div key={i} className="flex items-center gap-2 text-sm text-brand-100">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {categories.map((cat) => (
            <Card variant="glass" key={cat.label}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className={`h-3 w-3 rounded-full ${cat.color}`} />
                  <span className="font-medium text-white">{cat.label}</span>
                </div>
                <span className={`text-sm font-medium ${(cat.score ?? 0) >= 15 ? 'text-success-300' : (cat.score ?? 0) >= 10 ? 'text-warning-300' : 'text-brand-200'}`}>
                  {cat.score ?? '--'}/25
                </span>
              </div>
            </Card>
        ...
        <div className="space-y-3">
          {feedbackItems.map((item: any, i: number) => {
            ...
            return (
              <div key={i} className={`animate-slideUp rounded-lg border p-3 ${classes}`} style={{ animationDelay: `${i * 80}ms` }}>
  ...
  return (
    <PageTransition><div className="mx-auto max-w-2xl space-y-6">
      <h1 className="text-2xl font-bold text-white">CV Analysis</h1>
      <p className="text-sm text-brand-200">Upload your CV for AI-powered feedback.</p>
      ...
      {file && (
        <div className="text-center">
          ...
          {uploadMutation.isError && (
            <p className="mt-2 text-sm text-danger-400">{(uploadMutation.error as Error)?.message ?? 'Upload failed'}</p>
          )}
        </div>
      )}
    </div></PageTransition>
